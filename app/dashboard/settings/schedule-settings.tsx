...
export default function ScheduleSettings({ initialHours, isManualClosed: initialManualClosed, isDeactivated = false }: ScheduleSettingsProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-linear-to-r from-primary/5 to-transparent rounded-2xl border border-primary/10 transition-all duration-300 gap-4">
        <div className="space-y-0.5">
          <div className="flex items-center gap-2 text-wrap">
            <Label className="text-base font-bold">Status Toko (Manual)</Label>
            {isPending && <Loader2 className="h-3 w-3 animate-spin text-primary shrink-0" />}
          </div>
          <p className="text-xs text-muted-foreground leading-relaxed">Aktifkan untuk buka, matikan untuk tutup instan.</p>
        </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h4 className="text-sm font-bold uppercase tracking-widest text-muted-foreground">Jadwal Mingguan</h4>
          </div>
          <Button 
            type="button" 
            variant="outline" 
            ...
            className="rounded-xl border-primary/20 hover:bg-primary/5 h-9"
          >
        ...
        <div className={cn(
          ...
          <div className="p-4 sm:p-6 rounded-2xl bg-linear-to-br from-primary/10 to-transparent border border-primary/20 space-y-4">
            ...
            <div className="grid grid-cols-2 sm:flex sm:flex-wrap items-end gap-3 sm:gap-4">
              <div className="space-y-1.5">
                 <Label className="text-[10px] font-bold uppercase text-muted-foreground">Buka</Label>
                 <Input 
                   type="time" 
                   value={bulkOpen} 
                   onChange={(e) => setBulkOpen(e.target.value)}
                   className="w-full sm:w-32 bg-background h-10"
                 />
              </div>
              <div className="space-y-1.5">
                 <Label className="text-[10px] font-bold uppercase text-muted-foreground">Tutup</Label>
                 <Input 
                   type="time" 
                   value={bulkClose} 
          ...
          <div className="grid gap-3">
            {days.map((day) => (
              <div key={day} className={cn(
                 ...
                 schedule[day].is_closed ? "bg-muted/30 border-muted opacity-60" : "bg-card hover:border-primary/50 shadow-sm"
              )}>
                <div className="flex items-center justify-between sm:justify-start gap-4 min-w-[140px]">
                  <Label htmlFor={`schedule_${day}_active`} className="relative inline-flex items-center cursor-pointer group select-none">
                    <input 
                      ...
                      className="sr-only peer"
                    />
                    <div className="relative w-12 h-6.5 bg-muted rounded-full peer peer-checked:bg-primary transition-all duration-300 shadow-inner">
                      <div className={cn(
                    ...
                    <span className="ml-3 font-bold text-sm sm:text-base group-hover:text-primary transition-colors">{dayMap[day]}</span>
                  </Label>
                ...
                <div className="flex items-center gap-3">
                  <div className={cn(
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2">
                      <Label className="text-[10px] uppercase text-muted-foreground font-black sm:hidden">Jam Buka</Label>
                      <Label className="text-[10px] uppercase text-muted-foreground font-black hidden sm:block">Buka</Label>
                      <Input 
                        type="time" 
                        value={schedule[day].open || '08:00'}
                        onChange={(e) => updateTime(day, 'open', e.target.value)}
                        className="w-full sm:w-28 rounded-xl h-10 text-sm"
                      />
                    </div>
                    <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2">
                      <Label className="text-[10px] uppercase text-muted-foreground font-black sm:hidden">Jam Tutup</Label>
                      <Label className="text-[10px] uppercase text-muted-foreground font-black hidden sm:block">Tutup</Label>
                      <Input 
                        type="time" 
        ...
        {!isOpen && (
          <div className="grid grid-cols-2 min-[450px]:grid-cols-3 sm:flex sm:flex-wrap gap-2 text-[10px] font-black uppercase tracking-widest text-muted-foreground/60">
             {days.map(day => (
                <span key={day} className={cn(
                   "px-2 py-2 rounded-xl border text-center transition-all duration-300 shadow-xs",
                   schedule[day].is_closed ? "bg-muted border-muted/50" : "bg-primary/5 text-primary border-primary/20"
                )}>
